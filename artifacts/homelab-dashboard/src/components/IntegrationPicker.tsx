...
interface IntegrationPickerProps {
  ...
  value: string;
  // Fired with the chosen value; the picker closes itself afterwards.
...
export default function IntegrationPicker({
  ...
  // Reset the search box and scroll the current choice into view whenever the
  // picker reopens, so the highlighted card is visible immediately.
  useEffect(() => {
    if (!open) return;
    setQuery("");
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="flex max-h-[85vh] w-full max-w-2xl flex-col gap-3 p-0">
        <DialogHeader className="px-6 pt-6">
          ...
          <DialogDescription>
            Attach a service to show its live status, or pick None for a plain
            app shortcut.
          </DialogDescription>
        </DialogHeader>
