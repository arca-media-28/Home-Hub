...
export const GetTilesResponseItem = zod.object({
  ...
  "videoSource": zod.union([zod.literal('uploads'),zod.literal('urls'),zod.literal('youtube'),zod.literal('plex'),zod.literal('jellyfin'),zod.literal('ersatztv'),zod.literal(null)]).nullish().describe('Where a Video Player tile\'s videos come from: \"uploads\" (video files from the upload library), \"urls\" (a pasted list of direct video file URLs), \"youtube\" (a YouTube video\/playlist embed), \"plex\" or \"jellyfin\" (a library on the connected media server), or \"ersatztv\" (live TV channels from the connected ErsatzTV server). Null or absent means unconfigured — the tile plays the built-in yule log stream.'),
  ...
  "videoErsatzChannel": zod.string().nullish().describe('The tuned ErsatzTV channel number when videoSource is \"ersatztv\". Null or absent means the first channel in the lineup.'),
 ...
 */
export const ExportProfileQueryParams = zod.object({
  "includeConnections": zod.coerce.boolean().optional().describe('When true, include the user\'s saved service connection credentials (URLs, API keys, usernames, passwords, tokens) in READABLE form. Off by default.')
})

export const ExportProfileResponse = zod.object({
  "format": zod.string().describe('Constant identifier for this file type.'),
  "version": zod.number().describe('Schema version, bumped on incompatible format changes.'),
  "exportedAt": zod.string().optional().describe('ISO timestamp of when the file was produced.'),
  "deviceModes": zod.array(zod.object({
  "name": zod.string()
}).describe('A device mode inside a profile export, referenced by name.')),
  "pages": zod.array(zod.object({
  "name": zod.string(),
  "layoutPreset": zod.enum(['auto', 'adaptive', 'compact', 'fhd', 'qhd', 'uhd']).optional(),
  "layoutOrientation": zod.enum(['landscape', 'portrait']).optional(),
  ...
  "extra": zod.string().nullish().describe('Opaque JSON blob carrying service-specific extras (tokens, IMAP\/CalDAV account lists).')
}).describe('A service connection inside a profile export. ONLY present when the user explicitly opted in to including credentials — every field is stored and exported in readable plain text.')).nullish()
}).describe('A versioned envelope holding a user\'s entire profile — device modes, pages with per-mode\/variant tile layouts, and (only when explicitly requested) service connection credentials. The client additionally merges browser-side theme data into the downloaded file.')
 ...
 */
export const ImportProfileBody = zod.object({
  "format": zod.string(),
  "version": zod.number(),
...
}).describe('A profile export envelope plus the import mode. \"replace\" wipes the account\'s pages\/tiles\/device modes (and connections, if the file includes them) and recreates them from the file; \"merge\" appends pages and device modes with de-duplicated names.')

export const ImportProfileResponse = zod.object({
  "mode": zod.string(),
  "pages": zod.number(),
  "tiles": zod.number(),
  "deviceModes": zod.number(),
  "connections": zod.number()
}).describe('Counts of what an import created\/applied.')
...
export const GetTruenasMetricsResponse = zod.object({
  ...
  "cpuTempC": zod.number().nullish().describe('Current CPU temperature in °C (the hottest core), from the cputemp reporting graph; null when the box exposes no CPU temperature sensor or the reporting call is unavailable.'),
  "cpuTempCoresC": zod.array(zod.number()).optional().describe('Per-core CPU temperatures in °C for the current reading. Empty when no CPU temperature sensor is exposed or the reporting call is unavailable.'),
 ...
 */
export const GetErsatzChannelsResponse = zod.object({
  "sample": zod.boolean().nullish().describe('True when ErsatzTV is not connected; channels is demo data and no stream URLs exist (the tile keeps its built-in demo video).'),
  "channels": zod.array(zod.object({
