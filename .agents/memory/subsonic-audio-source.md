...
- **Write-back (scrobble)**: POST `/widgets/subsonic/scrobble` {id, submission?} proxies `scrobble.view` (submission=false = now-playing ping, true = completed play). Frontend `audioPlayer.tsx` drives it — detects the source from `ownerId === "audioplayer:subsonic"` (no source field on AudioTrack); now-playing on play/resume + 60s keep-alive interval, submission=true only from the `ended` handler (manual skips don't scrobble). All calls best-effort (`.catch` swallowed) so playback never breaks.
