...
- **Both orientations are fit-to-WIDTH now**: portrait originally fit-to-height (`min(heightScale, widthScale)`), which made the fold a hard content ceiling — any content past one screenful shrank the whole grid tiny with huge side padding. Locked scale is now `gridWidth/intrinsicGridWidth(cols)` for BOTH orientations; the reserving wrapper's `intrinsicHeight*scale` height grows with content so the page scrolls vertically. Fold guides are advisory-only visual hints in edit mode; nothing caps `gridY`.
