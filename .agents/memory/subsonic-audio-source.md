...
- Stream URL uses `/rest/stream.view?id=...&format=mp3&<auth>` so the shared `<audio>` engine plays any source codec; artwork uses `/rest/getCoverArt.view?id=<coverArt||albumId>&size=300&<auth>`. Subsonic has no live playback offset → progressMs/state stay null (now-playing entry gets state "playing").
