...
- The controls row's side elements (volume right, Find-music pill left) are absolutely positioned around the centered transport cluster (~92px), so each is gated on measured `density.bodyWidth` (geometry commented at the gating consts in `AudioPlayerTile.tsx`). Never gate them on density *level* alone — level is height-based and lets them overlap the transport buttons at narrow widths. Volume is a small round button opening a portalled Popover slider (user-requested after an inline slider either overlapped or vanished at medium widths); the pop-out can't collide, so only the ~28px button needs a width gate.
